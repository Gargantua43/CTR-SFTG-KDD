...
class MyDatasetTrain(Dataset):
    ...
    def __getitem__(self, index):
        # Visual1
        visual_csv_dir1 = self.df_Pair['Snap_Train_CTR_visual_pair1'][index]
        visual_data1 = np.loadtxt(visual_csv_dir1, delimiter=',')
        visual_train1 = torch.from_numpy(visual_data1[:, :])

        # Audio1
        audio_csv_dir1 = self.df_Pair['Snap_Train_CTR_audio_pair1'][index]
        audio_data1 = np.loadtxt(audio_csv_dir1, delimiter=',')
        audio_train1 = torch.from_numpy(audio_data1[:, :])

        # Text1
        text_csv_dir1 = self.df_Pair['Snap_Train_CTR_text_pair1'][index]
        text_data1 = np.loadtxt(text_csv_dir1, delimiter=',')
        text_train1 = torch.from_numpy(text_data1[:, :])

        # Visual2
        visual_csv_dir2 = self.df_Pair['Snap_Train_CTR_visual_pair2'][index]
        visual_data2 = np.loadtxt(visual_csv_dir2, delimiter=',')
        visual_train2 = torch.from_numpy(visual_data2[:, :])

        # Audio2
        audio_csv_dir2 = self.df_Pair['Snap_Train_CTR_audio_pair2'][index]
        audio_data2 = np.loadtxt(audio_csv_dir2, delimiter=',')
        audio_train2 = torch.from_numpy(audio_data2[:, :])

        # Text2
        text_csv_dir2 = self.df_Pair['Snap_Train_CTR_text_pair2'][index]
        text_data2 = np.loadtxt(text_csv_dir2, delimiter=',')
        text_train2 = torch.from_numpy(text_data2[:, :])

        # label
        pair_label = self.df_Pair['Train_CTR_Label'][index]
        label_data = np.array(pair_label)
        label_train = torch.from_numpy(label_data)

        return visual_train1, audio_train1, text_train1, visual_train2, audio_train2, text_train2, label_train
